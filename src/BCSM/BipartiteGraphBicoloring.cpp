...
namespace ColPack
{
...
#endif
                //cx Debug
                chrono::duration<double> tim = chrono::steady_clock::now() - t_start;
                //cout<<"runtime ";
                cout<<tim.count()<<endl;
		return(_TRUE);
	...
	string BipartiteGraphBicoloring::GetVertexBicoloringVariant()
	{
		...
		if(m_s_VertexColoringVariant.compare("DMD_COVER_STAR") == 0)
		{
			return("Dulmage-Mendelsohn-Decomposition Cover Star");
		}
		else
		{
			return("Unknown");
...
/*
	// ==========================================================
        // DM Decomposition
        // Author: Xin Cheng, 2020-Mar-14
	// ==========================================================
        

void BipartiteGraphBicoloring::DMDCoverVertex( 
            ...
            m_vi_IncludedLeftVertices .assign(M, 1);  // all included
            m_vi_IncludedRightVertices.assign(N, 1);  // all included
            m_vi_CoveredLeftVertices.clear();
            m_vi_CoveredLeftVertices.clear();
            {
                string line;
                getline(in, line);  // first line contains L and R information
                stringstream ss(line);
                int L_entries=0, R_entries= 0;
                ss >> L_entries >> R_entries;
                
                int vid=0;
                int n_read_entries=0; 
                const int n_expect_entries = L_entries + R_entries;
                // read left vertices
...
*/
        ...
        // ==========================================================
        // DM decomposition covering star bicoloring
        // Author : Xin Cheng , 2020-Mar-14
        // @nColors          : output, number of colors
        // @vColors_L : output, vertex color
        // @vColors_R : output, vertex color
        // @DMD_E_L       : input, DM Decomposition, vertex E
        // @tim         : ouput, runtime
        // @nVerbose    : input, verbose level
        // ==========================================================
	int BipartiteGraphBicoloring::DMDCoveringStarBicoloring(
               int& nColors
               , vector<int>& vColors_L
               , vector<int>& vColors_R
               , DMDecomposition const & DMD
               , int const nVerbose
               , chrono::duration<double>* pTim
               ) {

                chrono::steady_clock::time_point t_start;

		if(CheckVertexColoring("DMD_COVER_STAR"))
		{
			return(_TRUE);
		}
		...
		m_T_Timer.Start();
                ...
                // left size
		m_vi_CoveredLeftVertices.clear(); 
                m_vi_CoveredLeftVertices.assign(DMD.L.O.begin(), DMD.L.O.end());
                m_vi_CoveredLeftVertices.insert(m_vi_IncludedLeftVertices.end(), DMD.L.U.begin(), DMD.L.U.end());
                m_vi_IncludedLeftVertices.assign(i_LeftVertexCount+10, _TRUE); // all include
                for(auto v : DMD.L.E){
                    m_vi_IncludedLeftVertices[v] = _FALSE; // not include;
                }

                // right side
                m_vi_CoveredRightVertices.clear();
                m_vi_CoveredRightVertices.assign(DMD.R.O.begin(), DMD.R.O.end());
                m_vi_IncludedRightVertices.assign(i_RightVertexCount+10, _TRUE); // all include
                for(auto v : DMD.R.E){
                    m_vi_IncludedRightVertices[v] = _FALSE; // not include;
                }
                //DMDCoverVertex(DMDFileName, i_LeftVertexCount, m_vi_IncludedLeftVertices, m_vi_CoveredLeftVertices, i_RightVertexCount, m_vi_IncludedRightVertices, m_vi_CoveredRightVertices);
		...
		m_d_CoveringTime = m_T_Timer.GetWallTime();
                ...
                if(pTim!=nullptr) {  t_start = chrono::steady_clock::now(); }
		...
		//	cout<<"DEBUG 3559 | Star Bicoloring | Present Vertex | "<<STEP_UP(m_vi_OrderedVertices[i])<<endl;
			...
			if(m_vi_OrderedVertices[i] < i_LeftVertexCount)
			{
                ...
                // end to count the time
                if(pTim!=nullptr) { *pTim = chrono::steady_clock::now() - t_start; }
                ...
                // output
                nColors = m_i_VertexColorCount;
                vColors_L.assign(m_vi_LeftVertexColors.begin(), m_vi_LeftVertexColors.end()); 
                vColors_R.assign(m_vi_RightVertexColors.begin(), m_vi_RightVertexColors.end());
